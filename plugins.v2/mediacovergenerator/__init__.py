...
class MediaCoverGenerator(_PluginBase):
    ...
    # 插件描述
    plugin_desc = "生成媒体库动态/静态封面，支持 Emby/Jellyfin（补丁版）"
    # 插件图标
    plugin_icon = "https://raw.githubusercontent.com/justzerock/MoviePilot-Plugins/main/icons/emby.png"
    # 插件版本
    plugin_version = "0.9.5-patch1"
    # 插件作者
    ...
    def init_plugin(self, config: dict = None):
        ...
        if config:
            ...
            # 加载字体路径，过滤掉可能保存的 "None" 字符串（旧版本 bug）
            _zh_path = config.get("zh_font_path", "")
            _en_path = config.get("en_font_path", "")
            self._zh_font_path = _zh_path if _zh_path and _zh_path != "None" else ""
            self._en_font_path = _en_path if _en_path and _en_path != "None" else ""
    ...
    def __update_config(self):
        ...
        self.update_config({
            ...
            "zh_font_path": str(self._zh_font_path) if self._zh_font_path and str(self._zh_font_path) != "None" else "",
            "en_font_path": str(self._en_font_path) if self._en_font_path and str(self._en_font_path) != "None" else "",
...
''',
    ...
    def __generate_image_from_path(self, server, library_name, title, image_path=None, config_bg_color=None):
        ...
        # 检查字体路径是否有效
        if not self._zh_font_path or str(self._zh_font_path) == "None" or not self._en_font_path or str(self._en_font_path) == "None":
            ...
            if not using_local_font:
                ...
                if url_has_changed or not font_file_is_valid:
                    ...
                    if self.download_font_safely_with_timeout(url, downloaded_font_file_path):
                        ...
                        logger.warning(f"无法获取最新的{log_prefix}{lang}支持字体: {url}")
                        # 下载失败后，重新检查文件是否存在（download_font_safely不再提前删除）
                        if validate_font_file(downloaded_font_file_path):
                            logger.warning(f"下载失败，使用已缓存的有效字体文件 {downloaded_font_file_path}")
                            current_font_path = downloaded_font_file_path
                        else:
                            logger.critical(f"无法获取必要的{log_prefix}{lang}支持字体，且无缓存可用: {url}")
                            current_font_path = None
                else:
    ...
    def health_check(self) -> bool:
        ...
        try:
            ...
            # 检查字体文件
            zh_missing = not self._zh_font_path or self._zh_font_path == "None"
            en_missing = not self._en_font_path or self._en_font_path == "None"
            if zh_missing or en_missing:
                logger.warning("字体文件缺失，尝试重新获取")
                self.__get_fonts()

            # 重新获取后再次检查是否仍然缺失
            if not self._zh_font_path or self._zh_font_path == "None":
                logger.error("主标题字体获取失败，无法生成封面")
                return False

            if not self._en_font_path or self._en_font_path == "None":
                logger.error("副标题字体获取失败，无法生成封面")
                return False
    ...
    def download_font_safely(self, font_url: str, font_path: Path, retries: int = 2, timeout: int = 30):
        ...
        # 不在下载前删除现有文件，避免下载失败时丢失已有字体
        # 使用临时文件下载后再替换，确保原子性操作
